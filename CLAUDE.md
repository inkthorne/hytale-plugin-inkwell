...
- **Commit directly to `main` for ordinary work.** As a solo dev the feature-branch + PR ceremony isn't worth it most of the time — just keep every commit **coherent and the trunk releasable** (don't land a half-broken state; `main` HEAD must always build and load). Reach for a short-lived **feature branch + PR** (e.g. `feat/orbit-chicken`) only when a change is risky or large enough that you want it isolated or reviewable before it lands. No long-lived `dev` branch — `main` is the trunk.
